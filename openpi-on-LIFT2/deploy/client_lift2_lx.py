...
def finalize_runtime_args(args):
    ...
    if getattr(args, 'dry_run', False):
        ...
        args.enable_inference_executor = False
...
def format_launcher_summary(args):
    ...
    lines.extend([
        ...
        'Dry Run: ' + ('ON (no ROS command publish)' if args.dry_run else 'OFF'),
    ])
...
def main():
    ...
    rospy.loginfo(f"Dry run: {'Enabled' if args.dry_run else 'Disabled'}")
    ...
    if args.dry_run:
        rospy.loginfo("** DRY RUN: policy actions will be computed but not published **")
    rospy.loginfo("="*50)
