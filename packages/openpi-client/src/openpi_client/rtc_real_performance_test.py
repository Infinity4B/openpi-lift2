...
_DEFAULT_DATA_DIR = "./dataset_0403_cube"
_DEFAULT_DURATION_S = 10.0
_DEFAULT_CONTROL_PERIOD_S = 0.05
_DEFAULT_EXECUTION_HORIZON = 10
_DEFAULT_INFERENCE_DELAY = 2
_GAP_EPS_S = 1e-4
...
def _load_dataset_observation(data_dir: pathlib.Path, episode: int, frame: int, prompt: str) -> dict:
    ...
    if not episode_path.exists():
        pytest.skip(f"Dataset episode not found: {episode_path}")
...
def _run_non_rtc(
    ...
    *,
    ...
    while time.perf_counter() - benchmark_start_s < duration_s:
        ...
        chunk_execution_horizon = min(execution_horizon, action_chunk.shape[0])
...
def _plot_execution_gaps(ax, intervals: list[_Interval], y_pos: float) -> None:
    executions = sorted((interval for interval in intervals if interval.kind == "execution"), key=lambda i: i.start_s)
    for left, right in zip(executions, executions[1:], strict=False):
...
@pytest.mark.manual
def test_real_rtc_performance_with_dataset_observation():
    if os.environ.get("RUN_RTC_REAL_PERF") != "1":
        pytest.skip("Set RUN_RTC_REAL_PERF=1 to run the real RTC performance benchmark")

    host = os.environ.get("RTC_REAL_PERF_HOST", "localhost")
    port = _env_int("RTC_REAL_PERF_PORT", 8000)
    api_key = os.environ.get("RTC_REAL_PERF_API_KEY")
    data_dir = pathlib.Path(os.environ.get("RTC_REAL_PERF_DATA_DIR", _DEFAULT_DATA_DIR))
    prompt = os.environ.get("RTC_REAL_PERF_PROMPT", "perform task")
    episode = _env_int("RTC_REAL_PERF_EPISODE", 0)
    frame = _env_int("RTC_REAL_PERF_FRAME", 0)
    duration_s = _env_float("RTC_REAL_PERF_DURATION_S", _DEFAULT_DURATION_S)
    control_period_s = _env_float("RTC_REAL_PERF_CONTROL_PERIOD_S", _DEFAULT_CONTROL_PERIOD_S)
    action_horizon = _env_optional_int("RTC_REAL_PERF_ACTION_HORIZON")
    execution_horizon = _env_int("RTC_REAL_PERF_EXECUTION_HORIZON", _DEFAULT_EXECUTION_HORIZON)
    inference_delay = _env_int("RTC_REAL_PERF_INFERENCE_DELAY", _DEFAULT_INFERENCE_DELAY)
    output_dir = pathlib.Path(os.environ.get("RTC_REAL_PERF_OUTPUT_DIR", "./rtc_real_performance_outputs"))

    obs = _load_dataset_observation(data_dir, episode, frame, prompt)
    config = {
        "host": host,
        "port": port,
    ...
    non_rtc_requester = _TimedWebsocketRequester(host, port, "non_rtc", api_key=api_key)
    try:
    ...
    rtc_requester = _TimedWebsocketRequester(host, port, "rtc", api_key=api_key)
    try:
        rtc = _run_rtc(
            rtc_requester,
            obs,
    ...
    output_path = _write_outputs(output_dir, non_rtc, rtc, config)
    non_rtc_summary = _summarize(_relative_intervals(non_rtc))
    rtc_summary = _summarize(_relative_intervals(rtc))

    assert output_path.exists()
    assert non_rtc_summary["inference"]["count"] > 0
    assert rtc_summary["inference"]["count"] > 1
    assert rtc_summary["max_execution_gap_s"] < non_rtc_summary["max_execution_gap_s"]
